	...
	TileMap const &tilemap) const
{
	...
	iow::TileCoordi ncoord;
	GraphCost ncost;

	switch (directionTo) {
	case iow::Directions::UP:
		ncoord = iow::TileCoordi{coord.row - 1, coord.col};
	...
	case iow::Directions::LEFT:
		ncoord = iow::TileCoordi{coord.row, coord.col - 1};
		ncost = MOVEMENT_WEIGHT;
		break;
	...
	case iow::Directions::UP_RIGHT:
		ncoord = iow::TileCoordi{coord.row - 1, coord.col + 1};
	...
	case iow::Directions::INVALID_DIRECTION:
		...
		ncoord = iow::TileCoordi{coord.row, coord.col};
		ncost = MOVEMENT_WEIGHT;
		break;
	}
	...
	if (tilemap.getTileConfig(std::get<1>(tile)).collision) {
		return std::make_pair(tileMap.getTileIndex(ncoord),
				      MAX_GRAPH_COST);
	} else {
		return std::make_pair(tileMap.getTileIndex(ncoord), ncost);
	}
}
